...
CH_graph <- ggplot(data = CHplotting) + geom_point(mapping = aes( x = CHplotting[,1], y = CHplotting[,2]), size = 5, color = "red") + 
  ...
  theme(axis.title.x = element_blank(), axis.title.y = element_blank())
grid.arrange(CH_graph,plot_ch, ncol=2)
...
Ahora un ejemplo que con el CH sea complicado y con el SH se vea mejor
...
summary(X)
for (i in 1:S_index){
  S <- c(S,sum(select(filter(X, C_sh == i),c(5))))
  }
...
SH_graph <- ggplot(data = SHplotting) + geom_point(mapping = aes( x = SHplotting[,1], y = SHplotting[,2]), size = 5, color = "purple") + 
  geom_line( mapping = aes( x = SHplotting[,1], y = SHplotting[,2]), color = "green", size = 1) +
  labs(title = "Silhouette en función del número de clusters") + 
  theme(axis.title.x = element_blank(), axis.title.y = element_blank())
grid.arrange(SH_graph,plot_sh, ncol=2)
