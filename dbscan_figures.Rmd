...
grid.arrange(plot_dbs_1,plot_dbs_2,plot_real,ncol = 3, top = 'DBSCAN (MinPts = 15) en una partición con densidad variable')
```
...
plot_real <- ggplot(data = D)+geom_point(mapping = aes(x = D[,1], y = D[,2], color = class)) + labs(title = "Real Data Distribution", colour = "Clusters") + theme(axis.title.x = element_blank(), axis.title.y = element_blank())
grid.arrange(reach_plot,plot_real,ncol=2,top='Optics en un conjunto de densidad variable')
```