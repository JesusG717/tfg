...
```{r key realtions}
 ...
 track_part_final <- track_part_final %>% mutate(fifths = (2-mode)*key + (mode-1)*((key+3/11) %% 1))
 ggplot(track_part_final) + geom_density(aes(fifths,color = "Ciclo de quintas"),color = 'red') + geom_density(aes(key,color = "key"),color = 'green') + scale_color_manual(name = "Key vs Nueva variable", values = c("Ciclo de quintas" = "red", "key" = "green")) + theme_bw()
```
...
CH_plot+S_plot
```
Se observa como el indice CH se inclina más por 3 clusters, mientras que en el silueta se ve algo más de variedad. Consiguiendo números cercanos a 7 salvo con el metodo complete. En principio de momento vamos a suponer que hay 7 clusters (a corde con los generos), pero se intentará buscar significado a los valores altos de CH y el complete en silueta para k = 3. notar que es raro que el single de valores de k mas altos que el complete en el caso de silueta, cosa que intuitivamente no deberia pasar (chain effect)
...
```{r clustering}
k <- 7 #num clusters
km <- track_part_final %>% select(-c("track.id","genres")) %>% kmeanspp(k,nstart = 15)
...
plot_knn <- track_part_final %>% select(-c("track.id","genres")) %>%  kNNdist(k = 1) %>% sort() %>% data.frame() %>% mutate(N = c(1:nrow(plot_knn)))
knn_p <- ggplot(plot_knn,aes(N,.)) + geom_point() + theme_bw()

dbs <- track_part_final %>% select(-c("track.id","genres")) %>% dbscan(eps = 0.42, minPts = 5)
...
dbs_bars
```
Todos los algoritmos son capaces de separar la musica clásica, pero no diferencian bien los otros géneros. Además, el hecho de que los clusterings herarquicos ambos separen la musica clásica en varios grupos dice que estamos pidiendo demasiados grupos para nuestra intención. Ahora es necesario tratar de ver que hace que todos los generos no clásicos sean demasiado similares.
Además, el dbscan ahora mismo es un desastre.
...
Por tanto, ahora sería interesante comprobar esas dos hipótesis: se distinguiran mejor los generos sin estas variables? se confundira la musica clásica con el resto de generos al eliminar la loudness? hay que hacer mas clusterings para comprobarlo. 
